...
class WebController implements ControllerProviderInterface
{
    ...
    public function connect(Application $app)
    {
        ...
        $web->get('/letter/write', array($this, 'showWriteLetterPage'));
        $web->post('/letter/write', array($this, 'writeLetter'));

        $web->get('/letter/find', function () use ($app) {
            return $app['twig']->render('/find_letter.twig');
        });
        $web->post('/letter/find', array($this, 'findLetter'));

        $web->post('/letter/find/map', array($this, 'showFindLetterMapPage'));

        $web->post('/letter/read', array($this, 'showReadLetterPage'));
